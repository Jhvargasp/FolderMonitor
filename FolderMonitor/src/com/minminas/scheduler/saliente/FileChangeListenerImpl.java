...
public class FileChangeListenerImpl implements FileChangeListener {
	...
	private void asociaDocumentoFilenet(String absolutePath) {
		...
		if (ls != null && ls.size() > 0) {
			...
			String tipo = buscarDocPorTipo(store, name);
			if (tipo.equalsIgnoreCase("SALIENTE")) {
				avanzarFlujo(vwsess, name, (Document) ls.get(0));
			} else {
				avanzarFlujoCorrespondencia(vwsess, name, (Document) ls.get(0));
			}
		} else {
	...
	private String buscarDocPorTipo(ObjectStore store, String name) {
		SearchSQL sqlObject = new SearchSQL();
		String returnType = new String();
		sqlObject.setMaxRecords(100);
		String sql = bundle.getString("querySQLType");
		sql = sql.replaceAll("\\{FILENAME\\}", name);
		sqlObject.setQueryString(sql);
		log.debug(sql);
		SearchScope searchScope = new SearchScope(store);
		...
		if (!rowSet.isEmpty()) {
			for (Iterator it = rowSet.iterator(); it.hasNext();) {
				RepositoryRow element = (RepositoryRow) it.next();
				String idEl = element.getProperties().getStringValue("TipoComunicado");
				returnType = idEl;
			}
